...
1. **氛围编程失败的根本原因不是工具而是使用方式。** 常见场景：写出提示词 → 一路点接受 → 回头审查时发现代码难以审查、存在安全漏洞、风格不一致。问题不在于 AI 编程工具本身，而在于缺乏清晰的指令和可复用的工作模型。
...
#### 实践 1：保持上下文配置文件精简且保持更新

大多数 AI 生成代码的失败源于过时或过载的上下文。上下文窗口很快被填满，随着对话扩展输出质量下降。
...
**常见错误**：把配置文件当作文档来写，包含所有可能的决策说明。应该仅包含与该会话相关的指令。
...
配置文件应控制在 50 行以内。以下是精简有效的 `CLAUDE.md` 示例：
...
3. **`/rewind`**：回退到之前的检查点。会打开回退列表，可选择恢复代码和对话、仅恢复对话、仅恢复代码、摘要或直接返回。注意：`/rewind` 不是 `/clear` 的替代品——前者撤销操作，后者重置上下文。
...
不要让 AI 在没有任何计划审核的情况下生成代码。在 Claude Code 中，按 `Shift + Tab` 两次或使用 `/plan` 进入 Plan Mode。
...
> 务必在计划提示词中加上 **"List any assumptions"**。Claude 的假设往往是不匹配的源头，在生成代码前发现它们至关重要。

#### 实践 4：编写有范围和约束的提示词

每条提示词应包含：目标、约束列表、验证成功的指令。避免开放式提示词如 "Add error handling to my app"，这会导致 AI 到处添加错误处理。
...
一个好示例文件胜过一百字的风格描述——它精确展示了期望的功能在上下文中是什么样。
...
不要让 AI 直接写 schema 迁移，必须先产出书面计划，包含：前向变更 SQL、回滚路径、验证测试。
...
> 要求同时提供 up 和 down 迁移，确保迁移确实存在且在接触数据前已通过审查。
...
通过 `/skill-name`（如 `/write-tests`、`/review-code`）在 Claude Code 中直接运行。

**推荐在项目中构建的四类核心 Skill：**

1. **生成单元测试**：遵循项目框架和约定，覆盖边界和错误情况。每个 PR 前使用。
2. **安全检查遍**：检查暴露的敏感数据、未验证的用户输入、缺失的访问控制、注入攻击、API 密钥和环境变量的不安全使用。合并 API/认证/用户输入相关代码前使用。
...
**Skills 使用要点**：
- 同一提示词模式使用三到四次、明确最佳输出样式后再封装为 Skill
